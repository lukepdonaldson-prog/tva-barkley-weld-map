...
class DashboardViewTests(TestCase):
    ...
    def test_dashboard_renders_logout_as_post_form(self):
        ...
        self.assertContains(
            response,
            f'<form method="post" action="{reverse("logout")}" class="m-0">',
            html=False,
        )
        self.assertContains(
            response,
        ...
        self.assertNotContains(response, f'href="{reverse("logout")}"', html=False)
...
class PackagePortableScriptTests(SimpleTestCase):
    def test_package_script_includes_reports_app_directory(self):
        module_path = Path(__file__).resolve().parents[1] / 'scripts' / 'package_portable.py'
        spec = importlib.util.spec_from_file_location('package_portable', module_path)
        ...
        self.assertIn('reports', module._PROJECT_DIRS)
